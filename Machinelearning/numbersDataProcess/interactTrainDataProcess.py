...
from numbersDataProcess.paramsSet import cnn_set, rnn_set, test_cnn_set, test_rnn_set
from numbersInteraction.models import Users, NumbersModelBasicInfo, ValueSetData, NumbersLabelMap, NumbersTrainData, NumbersProcessData, NumbersCNNParams, NumbersRNNParams, NumbersKNNParams
from numbersDataProcess.dataload import build_vocab
    ...
    for item in train_data:
        try:
            ...
            for content in item["contents"]:
                ...
                contents.append(native_content(train_content))
                labels.append(native_content(label))
...
def knn_train_data(raw_data):
    ...
    if raw_train_data:
        ...
        try:
            NumbersLabelMap.objects.get(model_name=en_name)
            NumbersLabelMap.objects.filter(model_name=en_name).update(
                real_labels=db_real_labels,
                train_labels=db_train_labels
            )

        except Exception as e:
            db_operation = NumbersLabelMap(
                model_name=response,
                real_labels=db_real_labels,
                train_labels=db_train_labels
            )
        ...
        try:
            NumbersProcessData.objects.get(model_name=en_name)
            NumbersProcessData.objects.filter(model_name=en_name).update(
                contents=db_data,
                labels=db_labels,
            ...
            db_operation = NumbersProcessData(
                model_name=response,
                contents=db_data,
                labels=db_labels,
                data_count=data_count,
    ...
    try:
        NumbersKNNParams.objects.get(model_name=en_name)
        NumbersKNNParams.objects.filter(model_name=en_name).update(
            k=k
        )
    except Exception as e:
        db_operation = NumbersKNNParams(
            model_name=response,
            k=k
        )
        db_operation.save()
    ...
    NumbersModelBasicInfo.objects.filter(en_name=en_name).update(
        accuracy=acc,
        public_status=public_status,
        algorithm="KNN",
